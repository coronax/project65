...
; CJ - The program goes at the top of RAM, just below the XModem buffers
; so we can actually download it. We could experiment with moving it to
; the start of the program area instead, but adjust ccflag as needed.
Ram_base        = $0600             ; start of user RAM (set as needed, should be page aligned)
		; CJ 5600 gives us room so that we don't quite overlap the xmodem buffer area.
		; at 7f00
Ram_top         = $5600 ; $C000     ; end of user RAM+1 (set as needed, should be page aligned)
...
.word $5600                                     ; start address for loader

; cj BASIC reset point
RES_vec:
...
; CJ don't replace interrupt vectors. We need them for
; OS3 I/O.

; and put interrupt vectors in place
;        lda #<NMI_vec
;        sta $0201
;        lda #>NMI_vec
;        sta $0202
...
.pc02
P65_GetChar:
	phx             ; It's not entirely clear how much state these 
        phy             ; routines need to maintain. But the X register
	jsr $FFEb       ; definitely needs to be saved, probably Y as well.
	ply             ; Several places in the code also expect N or Z 
	plx             ; to reflect the received or written character.
	pha
	pla             ; Fix up status register
	rts
P65_PutChar:
	pha
	phx
	...
	plx
	pla
	;rts Let's just borrow the load/save RTS to save a byte
P65_Load:                                ; empty load vector for EhBASIC
P65_Save:                                ; empty save vector for EhBASIC
        RTS

; vector tables

LAB_vec:
        .word        P65_GetChar        ; GetChar$
        .word        P65_PutChar        ; PutChar$
        .word        P65_Load           ; null load vector for EhBASIC
        .word        P65_Save           ; null save vector for EhBASIC
.if 0 ;cj
; EhBASIC IRQ support
...
LAB_1295:
		; cj things seem to go wrong somewhere around ending a line and
		; starting a new one. is something awkward happening to the stack?
;		tsx
;		txa
;		jsr print_hex
...
LAB_1491:
; CJ this stack business looks suspicious. could this be interfering
; with the interrupt code? It's using a trick to pull two things off
; the stack and then "unpull" them by resetting the stack ptr. So we
; could conceivably overwrite those values. I think.
; Masking interrupts during this critical section seems to have fixed
; my crash-on-sending-a-file bug.
sei
...
LAB_1609:                       ; cj bug changed the square brackets below to parens
...
LAB_FBA2:
; cj don't think i want any interrupt checking.
